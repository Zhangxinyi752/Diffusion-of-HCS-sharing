globals [
  ...
  rewire-all?                          ; these two variables record which button was last pushed
  proportion
...
to setup
  ...
  wire-lattice
  ask n-of initial-sharers turtles
    [ become-adopted ]
  ...
  set a-values []
  set p-values n-values 5000 [random 10000000]
  set run-number 0
...
to set-up-nodes
   ...
   create-turtles num-HCS-owners [
    set color blue + 2
    set size 1
    set heading random 360
    become-susceptible
    set adopter-check-timer random adopter-check-frequency
  ]
...
to rewire-all
  ...
  if count turtles != num-HCS-owners [ setup ]
  ...
  set clustering-coefficient find-clustering-coefficient
  set average-path-length find-average-path-length
  set average-links find-average-node-links
  update-plots
end
...
to rewire-me ; turtle procedure
  ...
  if [ count link-neighbors ] of end1 < (count turtles - 1) [
    ...
    ; wire the new edge
    ask node-A [ create-link-with node-B [ set color 28.5 set rewired? true ] ]
...
to-report find-average-node-links
  let total-links 0
  ask turtles [
    set total-links total-links + count my-links
  ]
  let num-turtles count turtles

  report total-links / num-turtles
end
;;;;;;;;;;;;;;;;;;;;;
;; Edge Operations ;;
;;;;;;;;;;;;;;;;;;;;;
...
to wire-lattice
  ...
  while [ n < count turtles ] [
    ...
    ;make-edge turtle n
              ...
              ;"curve"
    ;make-edge turtle n
              ;turtle ((n + 7) mod count turtles)
              ;"curve"
    ;make-edge turtle n
              ;turtle ((n + 8) mod count turtles)
              ;"curve"
    set n n + 1
  ]
...
to calculate-proportion
  set proportion count turtles with [adopted?] / count turtles
  set participation-probability exp (-10.369 + 1.647 * price-premium-level / 0.25 + 0.477 * belonging + 0.961 * proportion / 0.2 + 1.391 * (rewiring-probability / (1 / 3)) ) / (1 + exp (-10.369 + 1.647 * price-premium-level / 0.25 + 0.477 * belonging + 0.961 * proportion / 0.2 + 1.391 * (rewiring-probability / (1 / 3)) ) )
end

to go
  if all? turtles [not adopted?] or proportion = 1
    [ stop ]
  set p-values lput proportion p-values
  if length p-values > 3000 [ set p-values butfirst p-values]
  if ticks > 2000  [if first p-values = last p-values [ stop ]]


  ask turtles
  [
    set adopter-check-timer adopter-check-timer + 1
    if adopter-check-timer >= adopter-check-frequency
        [ set adopter-check-timer 0]
  ]

  spread-part

  set last-proportion proportion
  calculate-proportion
  if last-proportion != 0 [set a (proportion * num-HCS-owners - last-proportion * num-HCS-owners )]
  set a-values lput a a-values
  if length a-values > 10 [set a-values butfirst a-values ]
  set b sum a-values
...
to spread-part
  ask turtles with [adopted? and adopter-check-timer = 0]
      [ ask link-neighbors with [not adopted?]
        [if random-float 100 < participation-probability
          [ become-adopted ] ] ]
end
...
to simulate-50-runs
  repeat 50 [
    ...
    while [ proportion < 0.99  and first p-values != p-values ] [ go ]
  ]
...
to setup-file
  file-open "6_0.7_0.5_1.5_data.csv"
  file-print "ticks,proportion,participation-probability,a,b,seed-value"
  file-close
end

to write-data-to-file
  file-open "6_0.7_0.5_1.5_data.csv"
  file-print (word (ticks) "," (proportion) "," (participation-probability) "," a "," b "," (seed-value))
  file-close
end
...
"" ""
PENS
"default" 1.0 2 -408670 true "" "plot participation-probability"
...
"" ""
PENS
"default" 1.0 1 -2382653 true "" "plot proportion"
